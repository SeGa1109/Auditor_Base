...
def AttendancePushLay():
    ...
    datalistnp = [[ms.Text("Emp. Code",justification= 'center',size=(15,1),relief= 'raised',font=fstylehd),
               ms.Text("Name",justification= 'center',relief= 'raised',size=(32,1),font=fstylehd),
               ms.Text("F/S Name",justification= 'center',relief= 'raised',size=(32,1),font=fstylehd),
               ms.Text("Shift_AT",justification= 'center',relief= 'raised',size=(20,1),font=fstylehd),
               ms.Text("OT Hours",justification= 'center',relief= 'raised',size=(10,1),font=fstylehd),
    ...
    mycursor.execute('select UID,emp_code,employee_name,f_sp_name from register where active_status = "Y" and ET = "PF" and shift_work = "Yes"')
    globals()['emplistpy']=[list(x) for x in mycursor.fetchall()]

    for i in range (len(emplistpy)):
        sub1=ms.Column([[ms.Input(emplistpy[i][1],font=fstyle,key='atpecsy'+str(i),size=(16,1),disabled=True,justification='center',
              ...
              ms.Text(emplistpy[i][3], font=fstyle, size=(35, 1) ,key='atpfssy'+str(i),justification='center'),ms.Sizer(35,0),
              ms.Radio("I", font=fstyle, key='atp1ssy' + str(i), group_id='atpsdsy' + str(i)),
              ms.Radio("II",font=fstyle,key='atp2ssy'+str(i),group_id='atpsdsy'+str(i)),
              ms.Radio("III",font=fstyle,key='atp3ssy'+str(i),group_id='atpsdsy'+str(i)),ms.Sizer(25,0),
              ms.Spin(values=[0,1,2,3,4],initial_value=0,font=fstyle,size=(8,1),key='atpotsy'+str(i),),ms.Sizer(15,0),
              ms.Input("0.0",font=fstyle,key='atpxpsy'+str(i),size=(15,1))
    ...
    for i in range (len(emplistpn)):
        sub1=ms.Column([[ms.Input(emplistpn[i][1],font=fstyle,key='atpecsn'+str(i),size=(16,1),disabled=True,justification='center',
                       disabled_readonly_background_color=ms.theme_background_color(),border_width= 0),ms.Sizer(5,0),
              ms.Text(emplistpn[i][2],font=fstyle,size=(35,1),key='atpnsn'+str(i),justification='center'),ms.Sizer(2,0),
              ms.Text(emplistpn[i][3], font=fstyle, size=(35, 1) ,key='atpfssn'+str(i),justification='center'),ms.Sizer(35,0),
              ...
              ms.Radio("A",font=fstyle,key='atp2ssn'+str(i),group_id='atpsdsn'+str(i)),
                         ms.Sizer(105,0),
              ms.Spin(values=[0,1,2,3,4],initial_value=0,font=fstyle,size=(8,1),key='atpotsn'+str(i),),ms.Sizer(15,0),
              ms.Input("0.0",font=fstyle,key='atpxpsn'+str(i),size=(15,1))
...
#---------------------------------------------------------------------
    ...
    for i in range(len(emplistny)):
        sub1 = ms.Column([[ms.Input(emplistny[i][1], font=fstyle, key='atpecnsy' + str(i), size=(16, 1), disabled=True,
                           ...
                           ms.Sizer(25, 0),
                           ms.Spin(values=[0, 1, 2, 3, 4], initial_value=0, font=fstyle, size=(8, 1),
                                   key='atpotnsy' + str(i), ), ms.Sizer(15, 0),
                           ms.Input("0.0", font=fstyle, key='atpxpnsy' + str(i), size=(15, 1))
    ...
    for i in range(len(emplistnn)):
        sub1 = ms.Column([[ms.Input(emplistnn[i][1], font=fstyle, key='atpecnsn' + str(i), size=(16, 1), disabled=True,
                                    justification='center',
                                    disabled_readonly_background_color=ms.theme_background_color(), border_width=0),
                           ms.Sizer(5, 0),
                           ...
                           ms.Sizer(105, 0),
                           ms.Spin(values=[0, 1, 2, 3, 4], initial_value=0, font=fstyle, size=(8, 1),
                                   key='atpotnsn' + str(i), ), ms.Sizer(15, 0),
                           ms.Input("0.0", font=fstyle, key='atpxpnsn' + str(i), size=(15, 1))
